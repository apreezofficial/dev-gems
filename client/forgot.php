...
<body class="bg-white dark:bg-gray-900 min-h-screen flex items-center justify-center transition-colors duration-300">
  <div class="max-w-5xl w-full mx-auto grid grid-cols-1 md:grid-cols-2 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl rounded-2xl shadow-lg overflow-hidden">
    ...
    <div class="p-8 sm:p-12 flex flex-col justify-center animate-fadeIn">
      <div class="max-w-sm mx-auto w-full">
        ...
        <h2 class="text-2xl sm:text-3xl font-bold text-black dark:text-white mb-2">Reset Your Password</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-8">Enter your email to receive a password reset link.</p>
        <form id="forgot-form" class="space-y-6">
          ...
          <button type="submit" class="w-full px-4 py-3 bg-black dark:bg-white text-white dark:text-black rounded-lg font-medium hover:bg-gray-800 dark:hover:bg-gray-200 transition-all duration-300 hover:scale-105">Send Reset Link</button>
        </form>
    ...
    <div class="hidden md:block relative bg-gray-100 dark:bg-gray-800 p-12">
      ...
      <div class="relative z-10 flex flex-col justify-center h-full animate-fadeIn delay-200">
        ...
        <p class="text-gray-600 dark:text-gray-400">Get back into your account quickly and securely with a reset link.</p>
      </div>
    </div>
  ...
  <script>
    ...
    forgotForm.addEventListener('submit', async (e) => {
      ...
      try {
        const response = await fetch('../server/auth/forgot.php', {
          method: 'POST',
